Placeholder.Kubernetes_operations Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: placeholder.kubernetes_operations.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from placeholder.kubernetes_operations
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: placeholder.kubernetes_operations.run
      vars:
        run_x: 42
```
